...
// Env sync: Supabase URL + keys all point to the kblsiyyelyokhxaxefhy project.
// SUPABASE_SERVICE_ROLE_KEY JWT ref matches the URL — resolveSupabaseUrl()
// derives the URL from the key's ref claim, so they must always stay in sync.
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    // Commons photo search — GET, auth + light rate limit.
    if (url.pathname === "/commons-photo" && request.method === "GET") {
      const guard = guardEndpoint(request, env, "/commons-photo", cors, env.RATE_LIMIT_KV);
      if (guard) return guard;
      return handleCommonsPhoto(request, cors);
    }
