     ...
     <div className="space-y-5 px-4 pb-8">
      ...
      <div>
        ...
        <div className="space-y-2">
          {KP_LEVELS.filter((kp) =>
            ...
            <SculptedCard key={kp.kpIndex} accent="cyan" className="flex items-start gap-3 p-3">
              <div className="glowing-border flex h-10 w-10 shrink-0 items-center justify-center rounded-full font-display text-lg font-bold text-white"
                style={{ backgroundColor: kp.color, ["--glow-color" as string]: kp.color }}>
                {kp.kpIndex}
              </div>
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2">
                  <h3 className="font-display text-sm font-bold text-foreground">Kp {kp.kpIndex} — {kp.level}</h3>
                  <TagChip accent={kp.color}>Visible ≥ {kp.visibility}</TagChip>
                </div>
                <p className="mt-1 text-xs text-[hsl(var(--text-mid))]">{kp.description}</p>
              </div>
            </SculptedCard>
          ))}
        </div>
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
        {VIEWING_TIPS.map((section) => (
          <SculptedCard key={section.title} accent="citrine" className="space-y-2 p-4">
      ...
      <SculptedCard accent="citrine" glowing className="p-5">
        <h2 className="mb-2 font-display text-base font-bold text-foreground">Live Forecast Resources</h2>
        <p className="mb-4 text-sm text-muted-foreground">For real-time aurora forecasts, check these trusted sources:</p>
