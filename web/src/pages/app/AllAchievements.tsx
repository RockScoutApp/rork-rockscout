...
export default function AllAchievements() {
  ...
  const { data: profile } = useQuery<{ level: number; xp: number }>({
  ...
  const { data: counts } = useQuery<AchievementState>({
    ...
    queryFn: async () => {
      if (!user)
        return { identify: 0, collection: 0, wishlist: 0, favorite_spots: 0, trips: 0, journal: 0 };
      const [col, wish, fav, trips, journal] = await Promise.all([
        supabase.from("rockscout_collection").select("id", { count: "exact", head: true }).eq("user_id", user.id),
        supabase.from("rockscout_wishlist").select("id", { count: "exact", head: true }).eq("user_id", user.id),
        supabase.from("rockscout_favorite_spots").select("id", { count: "exact", head: true }).eq("user_id", user.id),
        supabase.from("rockscout_trips").select("id", { count: "exact", head: true }).eq("user_id", user.id),
        supabase.from("rockscout_field_journal").select("id", { count: "exact", head: true }).eq("user_id", user.id),
      ]);
      return {
        identify: 0,
  ...
  const filtered = useMemo(() => {
    ...
    if (filter !== "ALL") result = result.filter((a) => a.metric === filter);
    if (search.trim()) {
      ...
      <div className="rounded-xl border border-border bg-gradient-to-br from-primary/10 to-card p-5">
        <div className="flex items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="font-display text-2xl font-bold text-foreground">Level {level}</p>
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {uniqueMetrics.map((m) => (
          <button
            ...
            onClick={() => setFilter(m)}
            className={`rounded-full px-3 py-1 text-xs font-medium transition-colors ${
              filter === m ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground hover:bg-muted/70"
            }`}
          >
            {METRIC_LABELS[m] ?? m}
          </button>
          ...
          return (
            <div
              ...
              className={`rounded-xl border p-4 transition-all ${
                earned ? "border-primary/40 bg-primary/5" : "border-border bg-card"
              }`}
            >
